...
import { AssignDepartmentAdminDialog } from "~/features/department-admin-grants/components/assign-department-admin-dialog";
import { ToggleDepartmentAdminGrantDialog } from "~/features/department-admin-grants/components/toggle-department-admin-grant-dialog";
import { useDepartmentAdminGrants } from "~/features/department-admin-grants/api/use-department-admin-grants";
...
/**
 * The FR43 delegation surface for a School Admin — grants/revokes Department Admin status for
 * coordinators in departments *within a school they administer*, via `POST/DELETE/GET
 * /departments/:departmentId/admin-grants` (delegable to a Coordinator who passes
 * `SchoolAdminOfDepartmentGuard`, unlike `DepartmentAdminGrantsPage`'s Super-Admin-only screen
 * which this shares its dialogs with). Direct structural mirror of
 * `DelegateModuleCreationPanel` (decision #34) one level up: the department picker is
 * intentionally local `useState`, not the `?departmentId=` URL-param pattern
 * `DepartmentAdminGrantsPage` uses (decision #32) — nothing deep-links into this tab yet.
 * Coordinator names resolved via the existing `useDepartmentCoordinators` hook (already
 * reachable by a School Admin, since `DepartmentAdminGuard` gained the School Admin
 * OR-condition backend-side) rather than the Super-Admin-only `useUsers()`.
 */
export function DelegateDepartmentAdminPanel() {
  ...
  const {
    ...
    isError,
    error,
  } = useDepartmentAdminGrants(departmentId);
  ...
  if (adminDepartments.length === 0) {
    return (
      <Alert
        ...
        message="You don't administer any department yet — this needs either a direct Department Admin grant or School Admin status over the department's school."
      />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold text-foreground">Delegate Department Admin</h2>
          <p className="text-sm text-muted-foreground">
            Let another coordinator manage every module in a department within a school you
            administer.
          </p>
        </div>
        <Button
      ...
      {isError && (
        <Alert
          ...
          status="error"
          timeout={0}
          title="Couldn't load Department Admin grants"
          message={
            ...
            {grantsLoading ? (
              <TableRow>
                <TableCell colSpan={4} className="h-24 text-center text-muted-foreground">
                  Loading Department Admin grants...
                </TableCell>
              </TableRow>
            ) : sortedGrants.length ? (
              ...
              <TableRow>
                <TableCell colSpan={4} className="h-24 text-center text-muted-foreground">
                  No Department Admin has ever been granted for this department.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      {selectedDepartment && (
        <AssignDepartmentAdminDialog
          ...
          onSuccess={(grant, apiMessage) => {
            ...
            showToast(
              apiMessage,
              `${coordinator?.fullName ?? "This coordinator"} can now manage every module in ${selectedDepartment.name}.`,
            );
          }}
        />
      )}

      <ToggleDepartmentAdminGrantDialog
        grant={toggleTarget}
        ...
        onSuccess={(action, grant, apiMessage) => {
          ...
          showToast(
            apiMessage,
            action === "revoked"
              ? `${name} no longer has Department Admin oversight here.`
              : `${name} has Department Admin oversight here again.`,
          );
        }}
      />
